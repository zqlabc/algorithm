...
public class Code06_SortStackWithRecursive {
    ...
    public static void sort(Stack<Integer> stack) {
        int deep = deep(stack);
        while (deep > 0) {
            int max = max(stack, deep);//找到栈deep深度的最大值
            int k = times(stack, deep, max);//统计最大值的次数
            down(stack, deep, max, k);//最大值下层到栈底，并且其他数的相对次序不变
            deep -= k;
        }
    }
    ...
    public static int max(Stack<Integer> stack, int deep) {
        ...
        int num = stack.pop();
        int resMax = max(stack, deep - 1);
        int max = Math.max(num, resMax);
        stack.push(num);
        return max;
    }
